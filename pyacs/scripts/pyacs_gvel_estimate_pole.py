...
def main():
    ...
    for plate in H_PLATE.keys():
        ...
        if (velplate.nsites() < len(linclude)):
            ...
            i = 0
            lmissing = [item for item in linclude if item not in velplate.lcode()]
        ...
        velplate.substract_pole(pole, type_euler='rot').write(name_out, verbose=verbose, comment='')

        name_out = args.experiment + '_res_' + plate
        main_log.write("-- writing results to: %s\n" % name_out)
        vel.substract_pole(pole, type_euler='rot').write(name_out, verbose=verbose, comment='')
    main_log.close()
