...
def main():
    ...
    parser.add_argument('--ncpu', action='store', type=int, dest='ncpu', default=None, help='number of CPUs to use.')
    ...
    if args.ncpu is None:
        ncpu = min(multiprocessing.cpu_count() - 1, 16)  # Use up to 8 CPUs, leave one free
    else:
        ncpu = args.ncpu
    ...
    if args.options_file:
        conf=Read_Conf_Pyacs.Conf(args.options_file)
    ...
    try:
        ...
        # print PYACS ERROR
        ERROR( ("Could not read: %s" % args.lsinex_name), exit=True )
    ...
    if args.ref_sinex_name is not None:
        ...
        try:
            ...
            ERROR(("Could not read: %s" % args.ref_sinex_name), exit=True)
    ...
    if args.ref_apr_name:
        ...
        try:
            ...
            ERROR(("Could not read: %s" % args.ref_apr_name), exit=True)
    ...
    if args.discontinuity is not None:
        ...
        try:
            ...
            ERROR(("Could not read: %s" % args.discontinuity), exit=True)
    ...
    if args.codomes is not None:
        ...
        try:
            ...
            ERROR(("Could not read: %s" % args.codomes), exit=True)
    ...
    if args.psd is not None:
        ...
        try:
            ...
            ERROR(("Could not read: %s" % args.psd), exit=True)
    else:
        psd = None
    ...
    for sinex_name in lsinex:
        ...
        free_sinex.read_section_estimate(discontinuity=discontinuities,rename=conf.rename,exclude=conf.exclude)
        ...
        try:
            ...
            (T,Residuals,H_stat)=\
                Helmert.estimate_helmert(H_common_Gpoint_ref,H_common_Gpoint_free,vcv_xyz=[],psd=psd,
                                ...
                                threshold=args.dikin_outlier,method=args.method)
        ...
        for i in np.arange(SORTED_RESIDUALS.shape[0]):
            ...
            if l_res_ts.has_ts(code):
                l_res_ts.__dict__[code].add_obs(date,[Rn,Re,Ru,1.E-3,1.E-3,1.E-3,0.,0.,0.],in_place=True,check=False)
            else:
                new_ts = Gts( code = code )
                new_ts.add_obs(date,[Rn,Re,Ru,1.E-3,1.E-3,1.E-3,0.,0.,0.],in_place=True,check=False)
        ...
        if isinstance(T,np.ndarray):
            pyacs.sol.log.helmert_residuals(Residuals, H_stat,free_sinex.name,sum_helmert_file)
        ...
        if ( H_COV is None ) and ( args.uncertainty ) and ( type_sol in ['snx','glx'] ):
            ...
            for sta in wsnx.sta:
                if sta.code in list(H_rename.keys()):
        ...
        snx_transformed=free_sinex.apply_helmert(T)

        ###########################################################################
        # UPDATE TIME SERIES
        ###########################################################################

        VERBOSE("updating time series from solution: %s" % (sinex_name) )
        ...
        for code, soln in sorted( snx_transformed.estimates.keys() ):
            ...
            # update gts
            sts.__dict__[ M.code ].add_obs_xyz(dec_date,XYZSXSYSZCXYCXZCYZ,in_place=True,check=False, neu=False)
    ...
    if replace:
        ...
        for code in sorted(sts.lcode()):
            sts_gts = sts.__dict__[code]
            ots_has_code = ots.has_ts(code)
            ots_gts = ots.__dict__[code] if ots_has_code else None
            worker_args.append((code, sts_gts, ots_has_code, ots_gts))
        ...
        with ProcessPoolExecutor(max_workers=ncpu) as executor:
            # Submit all tasks
            future_to_code = {executor.submit(merge_time_series_worker, args): args[0] 
                             for args in worker_args}
        
            # Process results with progress bar (if available) or simple counter
            if HAS_TQDM:
                with tqdm(total=len(worker_args), desc="Merging time series", 
                         unit="site", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]') as pbar:
                
                    for future in as_completed(future_to_code):
                        code, merged_gts, error = future.result()
                    
                        if merged_gts is not None:
                            successful_merges[code] = merged_gts
                            if not ots.has_ts(code):
                                MESSAGE("New time series added to solution: %s" % code)
                        else:
                            failed_merges[code] = error
                            ERROR("Failed to merge time series for %s: %s" % (code, error))
                ...
                for future in as_completed(future_to_code):
                    ...
                    if processed % 10 == 0 or processed == total:
                        MESSAGE("Processed %d / %d time series" % (processed, total))
    
        # Update the ots object with successful merges
        for code, merged_gts in successful_merges.items():
            ots.__dict__[code] = merged_gts
    
        MESSAGE("Successfully merged %d time series" % len(successful_merges))
        if failed_merges:
            MESSAGE("Failed to merge %d time series" % len(failed_merges))
    ...
    ###################################################################
    # GENERATE .data FROM .data_xyz
    ###################################################################

    ts1 = sts.gts_mp('xyz2neu',corr=True, ncpu=ncpu)
    ts2 = ts1.gts('reorder')
    final_sts = ts2.gts('correct_duplicated_dates',action='correct',tol= .1, in_place=True,verbose=False)
    ...
    if not pck_only:
        ...
        final_sts.gts('write_pos',path_pos)
    ...
    final_sts.write_pck( path_pck+'/'+args.expt+'.pck', verbose=verbose)
    ...
    if not pck_only:
        if sts.lcode() == []:
            ...
            if pyacs.verbose():
                sts.stat_site(save_dir=path_stat, display=True)
            else:
                sts.stat_site(save_dir=path_stat, display=False)
